...
async def dose_taken(reminder_id: str, user: dict = Depends(get_current_user)):
    res = supabase_admin.table("adherence").insert({
...
async def doses_upcoming(user: dict = Depends(get_current_user)):
    ...
    res = supabase_admin.table("adherence").select("*, reminders(*, medicines(*))").eq("user_id", user["user_id"]).eq("status", "pending").gte("scheduled_utc", today + "T00:00:00Z").lte("scheduled_utc", today + "T23:59:59Z").execute()
...
async def doses_history(user: dict = Depends(get_current_user)):
    res = supabase_admin.table("adherence").select("*").eq("user_id", user["user_id"]).order("scheduled_utc", desc=True).limit(50).execute()
    return SuccessResponse(data=res.data)
