...
✅ **Authentication & Session Management**: Supabase Auth integration with registration (`/auth/register`), login (`/auth/login`), silent refresh (`/auth/refresh`), 30-minute token-based email confirmation via Resend, and Two-Step TOTP Multi-Factor Authentication (MFA) step support. Resilient session handling against background transient 401s. JWT tokens stored in `sessionStorage` (not `localStorage`) to limit XSS exposure window.
✅ **Push Notifications & Reminders**: Reliable Web Push with VAPID keys delivered via the `dispatch-reminder` Supabase Edge Function (Deno + `web-push` npm package). Supports notification action buttons (Taken, Missed, Snooze), per-reminder tags for deduplication, idempotency check preventing re-send within 10 minutes, automatic cleanup of expired/unregistered push subscriptions (HTTP 404/410), and retry queue for transient delivery failures.
...
✅ **Dose & Adherence Tracking**: Log dose states (Taken, Missed, Snooze) with timestamp tracking and adherence percentage calculation. Auto-expiry pg_cron job marks overdue doses as missed with `ON CONFLICT DO NOTHING` race-condition guard.
✅ **Side-Effect Feedback**: Log side-effect records with 1–4 severity grading linked to specific medications and patient records. Severity-4 alerts trigger an `AFTER INSERT` PostgreSQL trigger that directly writes to `emergency_contact_alerts` — no Edge Function chain latency.
...
✅ **Security Hardening (v1.2.1)**: JWT role extraction from `app_metadata` only; CORS strict-origin enforcement; rate-limited feedback (5/hour); service-role isolation; security response headers (HSTS, CSP, X-Frame-Options); Vercel `maxDuration=60`; stale-reports recalculation cron; audit log index; `operational_state` view + `snooze_log` table.
